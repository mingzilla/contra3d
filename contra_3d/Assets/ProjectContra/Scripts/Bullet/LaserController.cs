...
namespace ProjectContra.Scripts.Bullet
{
    public class LaserController : MonoBehaviour
    {
        ...
        public static LaserController Spawn(Transform shotPoint, Vector3 positionDelta, bool isFacingForward, UserInput userInput, WeaponType weaponType, bool isOnGround)
        {
            GameObject prefab = AppResource.instance.GetBulletPrefab(weaponType);
            LaserController copy = Instantiate(prefab, (shotPoint.position + positionDelta), shotPoint.rotation).GetComponent<LaserController>();
